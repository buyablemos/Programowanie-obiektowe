...
#define DOKL_POROWNANIA 0.000001    // dokladnosc porownania liczb zespolonych 
...
LZespolona  LZespolona::operator / (const LZespolona & NR2)const
{
  ...
  double mianownik=( NR2.modul()*NR2.modul());
...
if(fabs(this->re - NR2.re) >= DOKL_POROWNANIA)
return false;

if(fabs(this->im - NR2.im) >= DOKL_POROWNANIA)
return false;
